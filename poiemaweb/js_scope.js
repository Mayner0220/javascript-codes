...
// 1. 스코프란?
// 스코프(scope, 유효범위)는 자바스크립트를 포함한 모든 프로그래밍 언어의 기본적인 개념으로 확실한 이해가 필요하다.
var x = "global";
...
// 전역에서 변수 x를 참조할 때, 그리고 함수 foo 내부에서 변수 x를 참조할 때 이름이 중복된 2개의 변수 중 어떤 변수를 참조해야 하는가?
// 자바스크립트는 어떻게 변수를 식별하는 것일까?
// 스코프는 참조 대상 식별자(indentifier, 변수, 함수의 이름과 같이 대상을 다른 대상과 구분하여 식별할 수 있는 유일한 이름)를 찾아내기 위한 규칙이다.
// 자바스크립트는 이 규칙대로 식별자는 찾는다.
...
// 즉, 전역에서 선언된 변수는 전역 스코프를 갖는 전역 변수이고, 지역에서 선언된 변수는 지역 변수가 된다.
// 전역 스코프를 갖는 전역 변수는 전역에서 참조할 수 있다.
// 지역에서 선언된 지역 변수는 그 지역과 그 지역의 하부 지역에서만 참조할 수 있다.

// 3. 자바스크립트 스코프의 특징
// 자바스크립트의 스코프는 타 언어와 다른 특징을 가지고 있다.
// 대부분의 C-family 언어는 블록 레벨 스코드(block-level scope)를 따른다.
// 블록 레벨 스코프란 코드 블록내에서 유효한 스코프를 의미한다.
// 여기서 "유효하다"라는 것은 "참조(접근)할 수 있다"라는 뜻 이다.
...
// 자바스크립트는 타 언어와 달리 특별한 시작점(entry point)이 없어서 위 코드와 같이 전역에 변수나 함수를 선언하기 쉽다.
// 자바스크립트는 다른 C-family 언어와는 달리 특별한 시작점이 없으며 코드가 나타나는 즉시 해석되고 실행된다.
// 따라서 전역에 변수를 선언하기 쉬우며 이것은 전역 변수를 남발하는 문제를 야기시킨다.
...
// 하지만 자바스크립트는 블록 레벨 스코프를 사용하지 않으므로 함수 밖에서 선언된 변수는 코드 블록 내에서 선언되었다 할지라도 모두 전역 스코프을 갖게 된다.
// 따라서 변수 x는 전역 변수이다.
...
// 전역 영역에서는 전역 변수만이 참조 가능하고 함수 내 지역 영역에서는 전역과 지역 변수 모두 참조 가능하나 위 예제와 같이 변수명이 중복된 경우, 지역 변수를 우선하여 참조한다.
// 다음 함수 내에 존재하는 함수인 내부 함수의 경우를 살펴보자.
var x = "global";
...
// 내부 함수는 자신을 포함하고 있는 외부 함수의 변수에 접근할 수 있다. 이는 매우 유용하다.
// 클로저에서와 같이 내부 함수가 더 오래 생존하는 경우, 타 언어와는 다른 움직임을 보인다.
// 함수 bar에서 참조하는 변수 x는 함수 foo에서 선언된 지역 변수이다.
// 이는 실행 컨텍스트의 스코프 체인에 의해 참조 순위에서 전역 변수 x가 뒤로 밀렸기 때문이다.
var x = 10;
...
// 내부 함수의 경우, 전역 변수는 물론 상위 함수에서 선언한 변수에 접근/변경이 가능하다.
var x = 10;
